...
/**
  ...
  * @tparam Pub
  */
case class ReactivePipe[In, Out, Sub[_]: AsSubscriber, Pub[_]: AsPublisher](input: Sub[In], output: Pub[Out]) {
...
object ReactivePipe {
  ...
  def multi[A](
    queueSize: Int,
    delayErrors: Boolean,
    minimumRequestedThreshold: Int = 10,
    subscriptionBatchSize: Int = 100)(
  ...
  class Instance[A](
    ...
    override protected val minimumRequestedThreshold: Int,
    override protected val subscriptionBatchSize: Int)(override implicit val ctxt: ExecutionContext)
      extends AsyncPublisher[A] with SingleSubscriber[A] {
