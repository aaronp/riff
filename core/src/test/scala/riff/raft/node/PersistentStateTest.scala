...
class PersistentStateTest extends RiffSpec {
  ...
  "PersistentState onRequestVote" should {
    ...
    "not grant a vote if we already cast a vote in the term" in {
      withPersistentState[String] { ps =>
        ...
        val originalTerm = ps.currentTerm
  ...
  def withPersistentState[A: ToBytes: FromBytes](test: PersistentState[A] => Unit) = {
    withClue("in memory") {
      test(PersistentState.inMemory[A]())

    }

    withClue("file based") {
      withDir { dir =>
        val ps = PersistentState[A](dir, true)
        test(ps)
      }
    }

    withClue("cached") {
      test(PersistentState.inMemory[A]().cached())
    }
  }
}
